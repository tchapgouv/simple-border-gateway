...
impl<H: GatewayHandler> hudsucker::HttpHandler for HandlerAdapter<H> {
    ...
    async fn handle_request(
        ...
        match convert_response_to_hudsucker(resp) {
            ...
            Err(e) => {
                return self
                    .handle_gateway_error(ctx, GatewayError::ConvertResponse(e.to_string()))
                    .await
                    .into();
            }
        }
    ...
    async fn handle_response(
        ...
        let resp = match convert_response_to_reqwest(resp) {
            ...
            Err(e) => {
                return self
                    .handle_gateway_error(_ctx, GatewayError::ConvertResponse(e.to_string()))
                    .await
            }
        };
        let resp = self
        ...
        match convert_response_to_hudsucker(resp) {
            Ok(res) => res,
            Err(e) => {
                self.handle_gateway_error(_ctx, GatewayError::ConvertResponse(e.to_string()))
                    .await
            }
        }
    }
...
fn convert_request(
    req: http::Request<hudsucker::Body>,
) -> Result<http::Request<reqwest::Body>, http::Error> {
...
fn convert_response_to_hudsucker(
...
) -> Result<http::Response<hudsucker::Body>, http::Error> {
    ...
    builder.body(hudsucker::Body::from_stream(stream))
}

fn convert_response_to_reqwest(
    resp: http::Response<hudsucker::Body>,
) -> Result<http::Response<reqwest::Body>, http::Error> {
    let (parts, body) = resp.into_parts();
    let mut builder = http::Response::builder().status(parts.status);
    for (name, value) in &parts.headers {
        builder = builder.header(name, value);
